...
const handlePaymentSuccess = async (paymentMethod) => {
  ...
  // ✅ IMPORTANT: backend expects flat IDs, not objects
  const foodItemIds = Object.keys(cart).flatMap(id =>
    Array(cart[id]).fill(parseInt(id))
  );

  try {
    // ✅ CORRECT REQUEST FORMAT (MATCHES YOUR BACKEND DTO)
   const orderRes = await axios.post(`${API}/orders`, {
  customerId: customerId ? parseInt(customerId) : 1,

  staffId: 1, // ✅ ADD THIS LINE

  restaurantTableId: diningLocation === 'cafeteria'
    ? selectedTableId
    : null,

  tableBookingId: diningLocation === 'cafeteria'
    ? tableBookingId
    : null,
  ...
  diningLocation: diningLocation || "room",
